...
export const AssetItemOnList = ({
  ...
  const {
    positions,
    amount: positionsAmount,
    currency: positionsAmountCurrency,
  } = useGetSTokenPositions(positionId);
  const { withdrawableReward, currency: withdrawableRewardCurrency } =
    useGetStokenRewards(positionId);
  const detectedPropertyAddress = useMemo(
    () => propertyAddress ?? positions?.property,
    [propertyAddress, positions]
  );
  ...
  const { myStakingAmount, currency: myStakingAmountCurrency } =
    useGetMyStakingAmount(propertyAddress);
  ...
  const stakedAmount =
    propertyAddress && !positionId ? myStakingAmount : positionsAmount;
  const stakedAmountCurrency =
    propertyAddress && !positionId
      ? myStakingAmountCurrency
      : positionsAmountCurrency;
  const stakingRewardAmount =
    propertyAddress && !positionId ? myStakingRewardAmount : withdrawableReward;
  const stakingRewardAmountCurrency =
    propertyAddress && !positionId
      ? myStakingRewardAmountCurrency
      : withdrawableRewardCurrency;

  return (
    <Wrap className={className}>
      <LinkWithNetwork
        href={"/[propertyAddress]"}
        as={`/${detectedPropertyAddress}`}
        passHref
      >
        <GridAvatar>
      ...
      <GridTotalStake
        ...
        value={
          (isPool
            ? toCurrency(myHolderAmount?.dp(2)).toNumber()
            : stakingRewardAmount?.dp(2).toNumber()) || 0
        }
      ...
      <GridButtons>
        {enableStake ? (
          <ButtonWithGradient onClick={onClick(onClickStake)}>
            Stake
          </ButtonWithGradient>
        ) : (
          ""
